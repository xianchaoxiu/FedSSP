...
from FLAlgorithms.users.userPAM import UserPAM
...
class AbnormalDetection(Server2):
    def __init__(self, algorithm, experiment, device, dataset, learning_rate, ro, beta, tau, lambda1,lambda2, num_glob_iters, local_epochs, clients, num_users, dim, time, exp_type,threshold,p,q,fileName):
        # 调用父类的初始化方法
        ...
        for i in range(self.num_clients):            
            ...
            if i == 0:
                ...
                temp_matrix = torch.rand(train.size(1), dim, dtype=torch.float)        # settings: 使用全一矩阵进分解
                Q, R = torch.linalg.qr(temp_matrix)
                self.commonPCAz = Q

                # self.commonPCAz = torch.rand(train.size(1), dim, dtype=torch.float)

                # 打印 commonPCAz 矩阵
                # print("commonPCAz matrix:")
                # print(self.commonPCAz)
            
            # user = UserADMM2(algorithm, device, id, train, self.commonPCAz, learning_rate, ro, local_epochs, dim)
            user = UserPAM(algorithm, device, id, train, self.commonPCAz, learning_rate, beta, tau, lambda1, lambda2, local_epochs, dim,p,q)
    ...
    def prep_data(self, dataX):
        ...
        change_dataX[featuresToScale] = change_dataX[featuresToScale].astype(float)  # 确保是 float 类型
        change_dataX.loc[:, featuresToScale] = sX.fit_transform(change_dataX[featuresToScale])  # 标准化
    ...
    def train(self):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            # # 判断是否满足停止准则
            # if len(losses_to_file) > patience and abs(losses_to_file[-1] - losses_to_file[-patience]) < loss_threshold:
            ...
            self.aggregate_pca_doubleSparse(self.beta,self.tau)  # 聚合PCA结果
...
#######################################################################################################
        # 保存公共表示、损失值和准确率到文件
        space = "Grassman-Sparse"
    
        directory = os.getcwd()
        res_name = "results"

        if self.dataset == "Unsw":
            data_path = os.path.join(directory, res_name+"_UNSW/" + self.fileName )   # data_path =  res_name+"/UNSW"
            ...
            data_path = os.path.join(directory, res_name+"_Iot23/"  + self.fileName)
            ...
            data_path = os.path.join(directory, res_name+"_Ton/" + self.fileName)
            ...
            data_path = os.path.join(directory, res_name+"_KDD/" + self.fileName)
        ...
        acc_file_name = f'{space}_T_{self.threshold}_dim_{self.dim}_clients_{self.num_clients}_frac_{self.user_fraction}_iters_{self.num_glob_iters}_localEp_{self.local_epochs}_lr_{self.learning_rate}_beta_{self.beta}_tau_{self.tau}_lambda1_{self.lambda1}_lambda2_{self.lambda2}_p_{self.p}_q_{self.q}'
        acc_file_path = os.path.join(acc_path, acc_file_name)


        
        losses_file_name = f'{space}_T_{self.threshold}_dim_{self.dim}_clients_{self.num_clients}_frac_{self.user_fraction}_iters_{self.num_glob_iters}_localEp_{self.local_epochs}_lr_{self.learning_rate}_beta_{self.beta}_tau_{self.tau}_lambda1_{self.lambda1}_lambda2_{self.lambda2}_p_{self.p}_q_{self.q}'

        losses_file_path = os.path.join(losses_path, losses_file_name)

        model_name = f'{space}_T_{self.threshold}_dim_{self.dim}_clients_{self.num_clients}_frac_{self.user_fraction}_iters_{self.num_glob_iters}_localEp_{self.local_epochs}_lr_{self.learning_rate}_beta_{self.beta}_tau_{self.tau}_lambda1_{self.lambda1}_lambda2_{self.lambda2}_p_{self.p}_q_{self.q}'
        ...
        # 保存实验指标
        metrics_file_name = f"{self.dataset}_{self.algorithm}_{self.fileName}.csv"
